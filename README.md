...
Все файлы правил синхронизированы по смыслу и версии (`bootstrap_version`, сейчас `v0.2`, см. `CHANGELOG.md`), а CI (`.github/workflows/checks.yml`) при каждом push/PR проверяет: синхронность указателей, соотношение версий (см. «Версионирование» ниже) и что JSON-манифесты (`plugin.json`, `marketplace.json`, `hooks/hooks.json`) валидны.
